...
from config import UNIT_TEST_PRIVATE_KEY, UNIT_TEST_PUBLIC_KEY
...
def secretsmanager_client(aws_credentials):
    with mock_secretsmanager():
        ...
        conn.create_secret(Name="test/otter/database",
                    SecretString='{"POSTGRES_PASSWORD": "example"}')
        conn.create_secret(Name="test/otter/private_key",
                    SecretString=UNIT_TEST_PRIVATE_KEY)
        conn.create_secret(Name="test/otter/public_key",
                    SecretString=UNIT_TEST_PUBLIC_KEY)
        yield conn

@pytest.fixture
def init_database():
    from backend.app.shared.client import DynamoDBClient
    ...
    def dynamodb_client():
        ...
        device = Device(
            ...
            common_name='ubuntu.example.com',
            ip_address='10.0.0.2',
            certificate_authority='lets_encrypt',
            data_center='DC0',
            ...
            device_model='None',
            origin='API',
            subject_alternative_name=['ubuntu.subdomain.example.com']
        )
        client.create_item(device)
...
def init_state():
    ...
    def stepfunction_client():
        conn = boto3.client("stepfunctions", region_name=AWS_REGION)
        conn.create_state_machine(
            name="otter-state", definition=str(definition), roleArn=_get_default_role()
        )
